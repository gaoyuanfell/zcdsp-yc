...
import { recursionChildCheck, recursionParentCheck, recursionResult, recursionResult2, recursionFilter } from '../../service/util';
...
export function directionalReducer(state: DirectionalState = initState, action: DirectionalActionUnion) {
  ...
  function setResult() {
    ...
    if (!state.audiencesAction || !state.audiencesAction2) return state
